...
Divide the route into segments, then route/plan each segment individually.

To maximize the amount of dirt, examine a lot of the route manually using satellite images and street view. Add surface and smoothness data to relevant roads on OSM, then have a look at the big picture using the Overpass API. To minimize zig-zagging , use BRouter with a custom dirt-finding profile to smoothly connect dirt segments.

* Potential Stops

- New Haven (start/end)
- [Decide on route between NHV and New Milford, but potentially:]
  - More west
    - Maltby / Racebrook
    - Pequonnock River Trail
    - Colis P. Huntington State Park
      - check official trail map, most but not all trails are multi-use
  - More east
    - West Rock
    - Naugatuck State Forest (Naugatuck)
    - Larkin State Park Trail
  - I think more east would be better and have less traffic. However, it would basically cut off the SW corner of CT.
...
Staring in New Haven, going clockwise:
1. Macedonia Brook State Park (~70mi)
2. American Legion State Forest (~50mi)
3. Bigelow Hollow State Park / Yale-Myers Forest (~65mi)
4. Pachaug State Forest (~50mi)
5. Gillette Castle (~60mi)
6. New Haven (~55mi)
...
Wrote a short script in JS to take the individual routing segments from BRouter (csv export, or directly on BRouter Web) and spit out a more useful breakdown of surface and terrain. It yields paved/unpaved distance and percentages, as well as more detailed breakdown of the specific surface (if available), percent bike path, etc.
